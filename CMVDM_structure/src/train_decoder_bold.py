...
def main(argv):
    ...
    train_labeled = BOLD5000Dataset(fmri_transform=fmri_xfm, phase='train')
    val_labeled = BOLD5000Dataset(fmri_transform=fmri_xfm, phase='test')

    n_voxels = train_labeled.num_voxels
    ...
    dec = make_model('BaseDecoder', n_voxels, im_res(), start_CHW=(8, 14, 14), n_conv_layers_ramp=FLAGS.n_conv_layers, n_chan=32,
        n_chan_output=n_chan_dec_output, depth_extractor=depth_extractor_dec)
    # dec = NewDecoder(n_voxels)
    ...
    criterions_dict = {
        ...
        'fmri': lambda pred, actual: 0.2*cosine_loss(pred, actual) + ((pred-actual).abs()).mean()
    }
    ...
    trainable_params = dec.parameters()
    ...
    optimizer = optim.Adam(dec.parameters(), lr=FLAGS.learning_rate, amsgrad=True)

    scheduler = None
    if FLAGS.scheduler > 0:
        if FLAGS.scheduler == 1:
            cprintm('(+) Using scheduler: On-Plateau.')
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.8, patience=10, verbose=True, cooldown=5)
    ...
    data_loaders_labeled = {
        ...
        'test': data.DataLoader(val_labeled, batch_size=min(batch_size_list()[-1], len(val_labeled)), shuffle=False, num_workers=num_workers(), pin_memory=True),
    }
            ...
            for epoch in range(FLAGS.n_epochs):
                ...
                if FLAGS.may_save:
                    if is_best: #or not FLAGS.savebestonly:
                        cprintc(f'Saving model...')
                        save_checkpoint({
                            'epoch': epoch + 1,
                            'state_dict': dec.state_dict(),
                            'best_loss': best_loss,
                            'optimizer': optimizer.state_dict(),
                        }, is_best, filepath=pjoin(exp_folder(), 'best_dec.pth.tar'))
                if FLAGS.verbose < 1:
                    pbar.update()
        # Report
    ...
    save_image(image_D_sbs_gt_train, pjoin(exp_folder(),'train.png'))
    save_image(image_D_sbs_gt_test, pjoin(exp_folder(),'test.png'))
...
def pure_test(data_loader_labeled, dec, save_folder='./vis_depth'):
    ...
    for batch_idx, (images_gt, fmri_gt) in enumerate(main_loader):
        ...
        for index, image_tensor in enumerate(pred_image_tensors):
            ...
            gt_depth = gt_image_tensors[index][3]
            save_image(torch.cat([gt_depth, image_tensor_depth], dim=1), pjoin(save_folder, str(index) + '_gray_depth.png'))
            save_image(gray2color(torch.cat([gt_depth, image_tensor_depth], dim=1)), pjoin(save_folder, str(index) + '_depth.png'))
            # import pdb;pdb.set_trace()

def run_test(dec_path):
    
    fmri_xfm = np.float32

    get_dataset = lambda subset_case: \
        KamitaniDataset('VC', sbj_num=3, im_res=112, fmri_xfm=fmri_xfm, subset_case=subset_case, select_voxels='', is_rgbd=1)

    train_labeled = get_dataset(KamitaniDataset.TRAIN)
    n_voxels = train_labeled.n_voxels
    print(n_voxels)
    import pdb;pdb.set_trace()
    val_labeled = KamitaniDataset('VC', sbj_num=3, fmri_xfm=fmri_xfm, subset_case=KamitaniDataset.TEST_AVG, select_voxels='', is_rgbd=1)
    ...
    val_labeled = CustomDataset(val_labeled, input_xfm=img_xfm_basic)
    depth_extractor_dec = None
    dec = make_model('BaseDecoder', n_voxels, 112, start_CHW=(64, 14, 14), n_conv_layers_ramp=3, n_chan=64, n_chan_output=4, depth_extractor=depth_extractor_dec)
    # Load pretrained encoder
    dec = nn.DataParallel(dec)
    ...
    data_loaders_labeled = {
        'train': data.DataLoader(train_labeled, batch_size=min([24,16,48,50][0], len(train_labeled)), shuffle=True, num_workers=7, pin_memory=True),
        'test': data.DataLoader(val_labeled, batch_size=min([24,16,48,50][-1], len(val_labeled)), shuffle=False, num_workers=7, pin_memory=True),
    }
