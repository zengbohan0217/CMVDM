...
def main(argv):
    ...
    train = BOLD5000Dataset(fmri_transform=fmri_transform, phase='train')
    val = BOLD5000Dataset(fmri_transform=fmri_transform, phase='test')

    global N_VOXELS #voxel_nc, 
    N_VOXELS = train.num_voxels
    # import pdb;pdb.set_trace()
    # voxel_nc = val.get_voxel_score('noise_ceil')

    # voxel_indices_nc_sort = np.argsort(voxel_nc)[::-1]
    # cprintm(u'(*) num_voxels: {} | noise ceiling {:.2f} \u00B1 {:.2f} (Mean \u00B1 SD)'.format(N_VOXELS, voxel_nc.mean(), voxel_nc.std()))

    if FLAGS.separable:
        model = make_model('SeparableEncoderVGG19ml', train.num_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
    else:
        model = make_model('BaseEncoderVGG19ml', train.num_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
    ...
    if FLAGS.init_cpt_name:
        ...
        print('\t==> Loading checkpoint {}'.format(init_cpt_path()))
    ...
    if FLAGS.scheduler > 0:
        if FLAGS.scheduler == 1:
            ...
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=10, verbose=True, cooldown=5)
        elif FLAGS.scheduler == 12345:  # Milestones
            milestones = [25, 35, 50, 70, 90]
            cprintm('(+) Using scheduler: tenth by milestones {} epochs.'.format(', '.join([str(x) for x in milestones])))
            scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma=FLAGS.gamma)
        else:
