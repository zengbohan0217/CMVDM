...
def create_Kamitani_dataset(path='../data/Kamitani/npz',  roi='VC', patch_size=16, fmri_transform=identity,
    ...
    train_text_token_path = os.path.join(path, 'GOD_train_texts_tokens.pth')
    test_text_token_path = os.path.join(path, 'GOD_test_texts_tokens.pth')
    ...
    train_tokens_and_texts = torch.load(train_text_token_path, map_location='cpu')
    test_tokens_and_texts = torch.load(test_text_token_path, map_location='cpu')
    ...
    test_fmri = np.concatenate(test_fmri, axis=0) # 50*4656
    train_fmri = np.concatenate(train_fmri, axis=0) # 1200*4656
    test_img = np.concatenate(test_img, axis=0) # 50*256*256*3
    train_img = np.concatenate(train_img, axis=0) # 1200*256*256*3
    ...
    if isinstance(image_transform, list):
        return (Kamitani_dataset(train_fmri, train_img, train_tokens_and_texts,  train_img_label_all, fmri_transform, image_transform[0], num_voxels, len(npz['arr_0'])), 
                Kamitani_dataset(test_fmri, test_img, test_tokens_and_texts, test_img_label_all, torch.FloatTensor, image_transform[1], num_voxels, len(npz['arr_2'])))
    else:
        return (Kamitani_dataset(train_fmri, train_img, train_tokens_and_texts, train_img_label_all, fmri_transform, image_transform, num_voxels, len(npz['arr_0'])), 
                Kamitani_dataset(test_fmri, test_img, test_tokens_and_texts, test_img_label_all, torch.FloatTensor, image_transform, num_voxels, len(npz['arr_2'])))
...
class Kamitani_dataset(Dataset):
    def __init__(self, fmri, image, tokens_and_texts, img_label, fmri_transform=identity, image_transform=identity, num_voxels=0, num_per_sub=50):
        super(Kamitani_dataset, self).__init__()
        self.fmri = fmri
        self.image = image
        self.tokens = tokens_and_texts['tokens']
        self.texts = tokens_and_texts['texts']
    ...
    def __getitem__(self, index):
        ...
        token = self.tokens[index].squeeze()
        ...
        if self.return_image_class_info:
            ...
            return {'fmri': self.fmri_transform(fmri), 'image': self.image_transform(img), 'token':token, 'text':text}
...
def create_BOLD5000_dataset(path='../data/BOLD5000', patch_size=16, fmri_transform=identity,
    ...
    train_text_token_path = os.path.join(path, 'BOLD5000_train_texts_tokens.pth')
    test_text_token_path = os.path.join(path, 'BOLD5000_test_texts_tokens.pth')
    train_tokens_and_texts = torch.load(train_text_token_path, map_location='cpu')
    test_tokens_and_texts = torch.load(test_text_token_path, map_location='cpu')
    ...
    fmri_train_major = np.concatenate(fmri_train_major, axis=0) # 4803*1696
    fmri_test_major = np.concatenate(fmri_test_major, axis=0) # 113*1696
    img_train_major = np.concatenate(img_train_major, axis=0) # 4803*256*256*3
    img_test_major = np.concatenate(img_test_major, axis=0) # 113*256*256*3

    num_voxels = fmri_train_major.shape[-1]
    if isinstance(image_transform, list):
        return (BOLD5000_dataset(fmri_train_major, img_train_major, train_tokens_and_texts, fmri_transform, image_transform[0], num_voxels), 
                BOLD5000_dataset(fmri_test_major, img_test_major, test_tokens_and_texts, torch.FloatTensor, image_transform[1], num_voxels))
    else:
        return (BOLD5000_dataset(fmri_train_major, img_train_major, train_tokens_and_texts, fmri_transform, image_transform, num_voxels), 
                BOLD5000_dataset(fmri_test_major, img_test_major, test_tokens_and_texts, torch.FloatTensor, image_transform, num_voxels))

class BOLD5000_dataset(Dataset):
    def __init__(self, fmri, image, tokens_and_texts, fmri_transform=identity, image_transform=identity, num_voxels=0):
        self.fmri = fmri
        self.image = image
        self.tokens = tokens_and_texts['tokens'] # nlist 1*77*64
        self.texts = tokens_and_texts['texts'] # nlist
        self.fmri_transform = fmri_transform
        self.image_transform = image_transform
        self.num_voxels = num_voxels
