...
from dc_ldm.ldm_for_fmri_control import fLDM
from eval_metrics import get_similarity_metric


def wandb_init(config, output_path):
    wandb.init( project='CMVDM',
                group="stageB_dc-ldm",
                anonymous="allow",
                config=config,
                reinit=True)
...
def main(config):
    ...
    generative_model = fLDM(pretrain_mbm_metafile, num_voxels,
                ...
                ddim_steps=config.ddim_steps, global_pool=config.global_pool, use_time_cond=config.use_time_cond, config_full=config)
...
def get_args_parser():
    ...
    parser.add_argument('--pretrain_finetune_path', type=str)
...
if __name__ == '__main__':
    ...
    output_path = os.path.join(config.root_path, 'results', f'{config.dataset}', 'control_generation',  '%s'%(datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S")))
