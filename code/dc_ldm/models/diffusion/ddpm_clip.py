...
from dc_ldm.models.fmri_base_decoder import text_clip_encoder
...
class DDPM(pl.LightningModule):
    ...
    def full_validation(self, batch, state=None):
        ...
        # grid, all_samples, state = self.generate(batch, ddim_steps=self.ddim_steps, num_samples=5, limit=20, state=state)
    ...
    def get_eval_metric(self, samples, avg=True):
        ...
        for s in samples_to_run:
            ...
            res = get_similarity_metric(pred_images, gt_images, 'class', None, 
                            n_way=50, num_trials=100, top_k=1, device='cuda')
            res_part.append(np.mean(res))
        res_list.append(np.mean(res_part))
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.clip_img_model, _ = clip.load("CLIP_path/ViT-B-32.pt", device='cuda', jit=False)

        # define the context extend encoder
        self.context_ext = text_clip_encoder()
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # cond_clip = self.context_ext.get_clip(cond)
        cond_clip = self.cond_stage_model.get_clip_feature(cond)
        cosine_feat = F.cosine_similarity(cond_clip.view(cond_clip.size(0), -1), clip_img_feature.view(cond_clip.size(0), -1), dim=1).mean()
        clip_loss = torch.exp(-cosine_feat).mean()
    ...
    def configure_optimizers(self):
        ...
        if self.train_cond_stage_only:
            ...
            # params = list(self.context_ext.parameters())
