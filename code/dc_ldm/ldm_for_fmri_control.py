...
class fLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        config = OmegaConf.load(config_full.config_root)
        ...
        if config_full.pretrain_finetune_path == None:
            pl_sd = torch.load(self.ckp_path, map_location="cpu")['state_dict']
        else:
            pl_sd = torch.load(config_full.pretrain_finetune_path, map_location='cpu')['model_state_dict']
        ...
        # here to load the stat_dict for controlnet
        model.model.diffusion_model.init_control_net()
        # model.model.diffusion_model.init_context_ext(model.context_ext.extend_deal)
    ...
    def generate(self, fmri_embedding, num_samples, ddim_steps, HW=None, limit=None, state=None):
        ...
        with model.ema_scope():
            ...
            for count, item in enumerate(fmri_embedding):
                ...
                fmri_depth = model.mimic_model(latent.to(self.device))
